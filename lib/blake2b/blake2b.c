...
static const uint64_t blake2b_IV[8] = {0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b,
...
static int blake2b(
    void* out, size_t outlen, const void* in, size_t inlen, const void* key, size_t keylen)
{
    blake2b_state S[1];
...
union ethash_hash512 ethash_blake2b512(const uint8_t* data, size_t size)
{
    union ethash_hash512 out;
    blake2b(out.bytes, sizeof(out), data, size, NULL, 0);
    return out;
}

union ethash_hash512 ethash_blake2b512_64(const uint8_t data[64])
{
    union ethash_hash512 out;
    blake2b(out.bytes, sizeof(out), data, 64, NULL, 0);
    return out;
}
